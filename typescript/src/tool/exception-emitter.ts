...
export function emitImportExceptions(generation: ImportGenerationRecord): string {
    ...
    const lines = ["import { PayloadException, RemoteException } from \"@cerasos/intercall\";", "", ""];
    for (const record of generation.exceptions) {
        ...
        if (fixedName !== undefined) {
            ...
            lines.push(`export const ${record.nativeName} = Object.freeze(new RemoteException(${JSON.stringify(record.declaration.name.name)}, ${declarationKey("exception", record.declaration.name.name)}n));`, "");
        } else {
