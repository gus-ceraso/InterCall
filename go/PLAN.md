...
`github.com/cerasos/intercall/go` and declares Go 1.26.5. Every command below uses
...
Bash: the Go commands from `go/` inside the task worktree, and the git-based
snapshot helpers (`task_status`, `task_snapshot*`) from the worktree root or
anywhere inside it.
...
complete gate and records exact results. The Go commands run from `go/` in the
task worktree; `task_snapshot_check` is git-based and works from the worktree
root:
